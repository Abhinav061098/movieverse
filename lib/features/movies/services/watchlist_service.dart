...
class WatchlistService extends ChangeNotifier {
  ...
  Future<Watchlist> createWatchlist(String name, String description) async {
    return await _operationLock.synchronized(() async {
      try {
        ...
        // toJson() already converts createdAt to created_at
        final json = watchlist.toJson();
        await watchlistRef.set(json);
        debugPrint(
            'WatchlistService: Successfully created watchlist $watchlistId with data: $json');
  ...
  Future<void> addToWatchlist(String watchlistId, MediaItem mediaItem) async {
    await _operationLock.synchronized(() async {
      ...
      try {
        final itemId = '${mediaItem.mediaType}_${mediaItem.id}';
        final itemData = {
          'id': mediaItem.id,
          'media_type': mediaItem.mediaType,
          'poster_path': mediaItem.item is Movie
              ? (mediaItem.item as Movie).posterPath
              : (mediaItem.item as TvShow).posterPath,
          'backdrop_path': mediaItem.item is Movie
              ? (mediaItem.item as Movie).backdropPath
              : (mediaItem.item as TvShow).backdropPath,
          'overview': mediaItem.item is Movie
              ? (mediaItem.item as Movie).overview
              : (mediaItem.item as TvShow).overview,
          'vote_average': mediaItem.item is Movie
              ? (mediaItem.item as Movie).voteAverage
              : (mediaItem.item as TvShow).voteAverage,
          'genre_ids': mediaItem.item is Movie
              ? (mediaItem.item as Movie).genreIds
              : (mediaItem.item as TvShow).genreIds,
          'added_at': DateTime.now().toIso8601String(),
        };

        // Add media type specific fields
        if (mediaItem.mediaType == 'tv') {
          itemData['name'] = (mediaItem.item as TvShow).title;
          itemData['first_air_date'] = (mediaItem.item as TvShow).firstAirDate;
        } else {
          itemData['title'] = (mediaItem.item as Movie).title;
          itemData['release_date'] = (mediaItem.item as Movie).releaseDate;
        }

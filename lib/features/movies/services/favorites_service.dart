...
class FavoritesService extends ChangeNotifier {
    ...
    while (true) {
      try {
        ...
        if (retryCount >= _maxRetries) {
          debugPrint(
              '$operationName failed after $_maxRetries attempts: $e\n$stack');
          throw FavoriteOperationFailedException(
              '$operationName failed after $_maxRetries attempts', e);
        }
        debugPrint(
            '$operationName failed (attempt $retryCount): $e\nRetrying in ${_retryDelay.inSeconds}s...');
  ...
  Future<bool> toggleMovieFavorite(int movieId,
    ...
    return await _operationLock.synchronized(() async {
      final userId = _auth.currentUser?.uid;
      if (userId == null) {
        throw FavoriteOperationFailedException('User not logged in');
      }
